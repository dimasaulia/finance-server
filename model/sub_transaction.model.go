...
func (t *SubTransaction) DeleteSubTransaction(tx *gorm.DB) error {
	// Cek Apakah Sub Transaksinya Masih Ada
	subTransaction := new(SubTransaction)
	qSubTransaction := tx.Model(&SubTransaction{}).Select("*").Where("id_sub_transaction", t.IdSubTransaction).Where("id_user", t.IdUser).First(subTransaction)
	if qSubTransaction.Error != nil {
	...
	if isNewerSubTransactionExist {
		return fmt.Errorf("this sub transaction cannot be deleted because a newer transaction exists. please delete the latest sub transaction before making changes")
	}

	err := tx.Where("id_sub_transaction", subTransaction.IdSubTransaction).Where("id_user", subTransaction.IdUser).Delete(&SubTransaction{}).Error
	if err != nil {
